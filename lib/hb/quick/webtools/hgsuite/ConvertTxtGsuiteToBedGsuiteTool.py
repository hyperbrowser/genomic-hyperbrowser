...
class ConvertTxtGsuiteToBedGsuiteTool(GeneralGuiTool):

    NUM_EXAMPLE_LINES = 12
    NUM_PARAM_BOXES = 10

    ALL_PUBLIC_OPERATIONS = OrderedDict(
        [('Convert txt to 3-column bed', 'ConvertTxtTo3ColBed')])
    ...
    def getOptionsBoxFirstLinesOut(cls, prevChoices):
        operation = cls.ALL_OPERATIONS.keys()[0]
        try:
            return cls._runOperationOnExampleDataAndReturnOutput(
                prevChoices), cls.NUM_EXAMPLE_LINES, True
        except:
            pass
    ...
    def execute(cls, choices, galaxyFn=None, username=''):
        ...
        for track in gSuite.allTracks():
            ...
            fileName = changeSuffixIfPresent(fileName, oldSuffix=track.suffix,
                                             newSuffix=newSuffix)
