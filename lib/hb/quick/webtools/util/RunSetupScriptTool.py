...
class RunSetupScriptTool(GeneralGuiTool):
    RELOAD_TIME = 3
    SETUP_PREFIX_FROM_URL_PREFIX = {
        'hb': 'stable',
        'dev2': 'developer'
    }
    ...
    def execute(cls, choices, galaxyFn=None, username=''):
        ...
        import os
        from config.Config import URL_PREFIX, HB_SETUP_CONFIG_BASE_DIR, \
            HB_SETUP_CONFIG_DEFAULT_FN

        setupFileFn = HB_SETUP_CONFIG_DEFAULT_FN
        if '/' in URL_PREFIX:
            setupPrefixCand = URL_PREFIX.split('/')[1]
            if setupPrefixCand in cls.SETUP_PREFIX_FROM_URL_PREFIX:
                setupPrefixCand = cls.SETUP_PREFIX_FROM_URL_PREFIX[setupPrefixCand]
            setupFileCand = setupPrefixCand + '.setup'

            customSetupDir = os.path.join(HB_SETUP_CONFIG_BASE_DIR, 'custom')

            if setupFileCand in os.listdir(customSetupDir):
        ...
        with open(galaxyFn, 'a', 0) as outFile:
            ...
            print>>outFile, str(core)

            from setup.InstallFunctions import executePythonFile
            executePythonFile(os.sep.join(['setup.py']),
                              args='-s %s' % setupFileFn, cwd=HB_SOURCE_CODE_BASE_DIR,
                              onError='nothing', stdout=outFile, stderr=outFile, noBuffer=True)
