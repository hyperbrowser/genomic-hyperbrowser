...
class CreateGSuiteFromTwoBinomalDistributionsTool(GeneralGuiTool):
    @classmethod
    def getToolName(cls):
    ...
    def execute(cls, choices, galaxyFn=None, username=''):
        ...
        outGSuite = cls._countResults(gSuite, gtrackData, firstProb, secondProb, number, galaxyFn)
    ...
    def _countResults(cls, gSuite, gtrackData, firstProb, secondProb, number, galaxyFn):
        ...
        for i, iTrack in enumerate(gSuite.allTracks()):
            ...
            for nr in range(0, number):
                dataset = cls._countAllForRequal01(datasetPerChromosome, gtrackData, firstProb, secondProb, number)
                if len(dataset) > 0:
                    cls._buildTrack(outGSuite, trackTitle, gSuite.genome, dataset, galaxyFn, nr)
    ...
    def _buildTrack(cls, outGSuite, trackTitle, genome, dataset, galaxyFn, nr):
        ...
        gs = GSuiteTrack(uri, title=''.join(trackTitle + '--' + str(nr)), genome=genome, attributes=attr)

        outGSuite.addTrack(gs)


    @classmethod
    def _countAllForRequal01(cls, datasetPerChromosome, gtrackData, firstProb, secondProb, number):

        allPossibilitiesWithOptionFinalChoice = []
        for chr in datasetPerChromosome.keys():
            allPossibilitiesWith1 = datasetPerChromosome[chr]
            allPossibilitiesWith0 = [i for i in range(gtrackData[chr][0], gtrackData[chr][1] + 1)]
            allPossibilitiesWith0 = [x for x in allPossibilitiesWith0 if
                                     x not in datasetPerChromosome[chr]]

            allPossibilitiesWith1BinomalDistribution = cls._countBinomalDistribution(n=len(allPossibilitiesWith1), size=1, prob=firstProb)
            allPossibilitiesWith0BinomalDistribution = cls._countBinomalDistribution(n=len(allPossibilitiesWith0), size=1, prob=secondProb)

            cls._selectChoiceWith1(chr, allPossibilitiesWith1BinomalDistribution, allPossibilitiesWith1, allPossibilitiesWithOptionFinalChoice)
            cls._selectChoiceWith1(chr, allPossibilitiesWith0BinomalDistribution, allPossibilitiesWith0, allPossibilitiesWithOptionFinalChoice)

        return allPossibilitiesWithOptionFinalChoice

    @classmethod
    def _selectChoiceWith1(cls, chr, allPossibilitiesWithOptionBinomalDistribution, allPossibilitiesWithOption, finalList):

        for i, elI in enumerate(allPossibilitiesWithOptionBinomalDistribution):
            if elI == 1:
                finalList.append([chr, str(allPossibilitiesWithOption[i]), str(allPossibilitiesWithOption[i]+1)])
