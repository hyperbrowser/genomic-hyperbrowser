...
def addColumnPlotToHtmlCore(htmlCore, seriesName, categories,
    ...
    #hicharts doesn't handel None values
...
def getRawAndDerivedOverlapResultsForTrackVsCollection(targetTrackGenome, targetTrack, referenceTracksGSuite, analysisBins=None):
    analysisSpec = AnalysisSpec(RawOverlapStat)
...
def getOverlapAndEnrichmentValuesFromResultsDict(rawAndDerivedOverlapResultsData, overlapTableTitle, enrichmentTableTitle, galaxyFn):
    overlapCoverageList = [(x['raw'].getGlobalResult()['Both'], x['raw'].getGlobalResult()['Both'] + x['raw'].getGlobalResult()['Only2']) 
                   for x in rawAndDerivedOverlapResultsData.values()]
    exonOverlapTableData = OrderedDict([(key, toPercentageValue(val['raw'].getGlobalResult()['Both'], val['raw'].getGlobalResult()['Both'] + val['raw'].getGlobalResult()['Only2'])) 
                                        for (key, val) in rawAndDerivedOverlapResultsData.iteritems()])
    exonOverlapTableLink = generateTableURLFromDataDict(galaxyFn, overlapTableTitle.replace(' ', '_'), ['Track', 'Overlap proportion with exons (Ensembl) in %'], exonOverlapTableData, plotType='columnChart')
    ...
    exonEnrichmentTableData = OrderedDict([(key, val['derived'].getGlobalResult()['2in1']) for (key, val) in rawAndDerivedOverlapResultsData.iteritems()])
    exonEnrichmentTableLink = generateTableURLFromDataDict(galaxyFn, enrichmentTableTitle.replace(' ', '_'), ['Track', 'Enrichment in exons (Ensembl)'], exonEnrichmentTableData, plotType='columnChart')
...
#     print '<br \>'
    ...
    if par == 1:
        ...
        htmlCore.tableLine(['Unique base-pair coverage (elements)', vals[0], vals[1], vals[3], vals[4], generateImgLink(vals[20], 'Show histogram'), generateImgLink(vals[2], 'Show detailed plot')])
        htmlCore.tableLine(['Genome fraction coverage (bps)', vals[5], vals[6], vals[8], vals[9], generateImgLink(vals[21], 'Show histogram'), generateImgLink(vals[7], 'Show detailed plot')])
        htmlCore.tableLine(['Genome fraction coverage (%)', vals[10], '', vals[12], vals[13], generateImgLink(vals[22], 'Show histogram'), generateImgLink(vals[11], 'Show detailed plot')])
#         htmlCore.tableLine(['Average contig coverage (%)', '', '', vals[14]+'-'+vals[15], vals[16]+'-'+vals[17], '-', '-'])
        ...
        htmlCore.tableLine(['Unique to that track (not present in any of the other '+vals[2]+' tracks)', vals[0]+'%', '~'+vals[1]])
...
#     DebugUtil.insertBreakPoint()
    if plotType!=None:
        ...
        if plotType == 'columnCharts':
            ...
            seriesName, categories, data = dT.changeDictIntoList()
            res = vg.drawPieChart(data, seriesName=categories, titleText=tableHeader[1],  addOptions='width: 50%; margin: 0 auto')
...
#     table3 = generateHistogram('propCoverage', ['Track', 'Histogram for genome fraction coverage (%)'], tableData3, plotType='columnChart')
    ...
    table1LinkHist = generateHistogramURLFromDataDict(galaxyFn, 'histElementCount', ['Track', 'Histogram for unique base-pair coverage (elements)'], tableData1, plotType='columnChart')
    table2LinkHist = generateHistogramURLFromDataDict(galaxyFn, 'histCoverage', ['Track', 'Histogram for genome fraction coverage (bps)'], tableData2, plotType='columnChart')
    table3LinkHist = generateHistogramURLFromDataDict(galaxyFn, 'histPropCoverage', ['Track', 'Histogram for genome fraction coverage (%)'], tableData3, plotType='columnChart')
    
    
    
    table1Link = generateTableURLFromDataDict(galaxyFn, 'elementCount', ['Track', 'Element count'], tableData1, plotType='columnChart')
    table2Link = generateTableURLFromDataDict(galaxyFn, 'coverage', ['Track', 'Coverage in bps'], tableData2, plotType='columnChart')
    table3Link = generateTableURLFromDataDict(galaxyFn, 'propCoverage', ['Track', 'Proportional coverage'], tableData3, plotType='columnChart')
    
     
    tableData, regionNames = gSOData.getPropCoveragePerRegionDataDict()
    table4Link = generateTableURLFromDataDict(galaxyFn, 'propCoveragePerChr', ['Track'] + regionNames, tableData, plotType='columnCharts')
#     table4 = generateHistogram('propCoveragePerChr', ['Track', 'Histogram'], tableData, plotType='columnChart')
    ...
    table5Link = generateTableURLFromDataDict(galaxyFn, 'elementCountPerChr', ['Track'] + regionNames, tableData, plotType='columnCharts')
...
#     paragraphs.append(table3)
    ...
    table6Link = generateTableURLFromDataDict(galaxyFn, 'avgSegLen', ['Track', 'Average segment length'], tableData, plotType='columnChart')
...
#Exons, introns overlap
    if gSuite.genome == 'hg19':
        exonsRawAndDerivedOverlap, intronsRawAndDerivedOverlap = getExonIntronRawAndDerivedOvelapData(gSuite, analysisBins=userBin)
        ...
        = getOverlapAndEnrichmentValuesFromResultsDict(exonsRawAndDerivedOverlap, 'Overlap with exons (Ensembl)', 'Enrichment in exons (Ensembl)', galaxyFn)
        ...
        overlapTableLink = generateTableURLFromDataDict(galaxyFn, 'Overlap', 
                                                                   ['Track', 
    ...
    if gSuite.genome == 'hg19':
        ...
        commonRepeatsOverlapResults = getCommonRepeatsRawAndDerivedOverlapData(gSuite, analysisBins=userBin)
        ...
        = getOverlapAndEnrichmentValuesFromResultsDict(commonRepeatsOverlapResults, 'Overlap with repeats', 'Enrichment in repeats', galaxyFn)
        
        allRepeatsOverlapResultsDict = getAllRepeatsProportionOverlapData(gSuite, analysisBins=userBin, username=username)
        
        allRepeatsOverlapTableLink = generateTableURLFromDataDict(galaxyFn, 'repeatsOverlapProportion', ['Repeats track'] + gSuite.allTrackTitles() , allRepeatsOverlapResultsDict, plotType='columnCharts')
...
#     htmlCore.tableFooter()
    if expandable and len(tableData) > visibleRows:
        htmlCore.tableFromDictionary(tableData, headerRowStats, tableId='gsOverviewTable', expandable=expandable, visibleRows=visibleRows)
    else:
        htmlCore.tableFromDictionary(tableData, headerRowStats, tableId='gsOverviewTable', expandable=False)
    ...
    headerRow = ['Base-pair coverage average', 'Segment count average','Base-pair coverage min', 'Base-pair coverage max']
    htmlCore.tableHeader(headerRow)
    htmlCore.tableLine([overviewData.avgCoverage, overviewData.avgElementCount, overviewData.minBpsCoverage, overviewData.maxBpsCoverage])
    htmlCore.tableFooter()
    htmlCore.divEnd()
...
def getExonIntronRawAndDerivedOvelapData(gSuite, analysisBins=None):
    ...
    exonsOverlap = getRawAndDerivedOverlapResultsForTrackVsCollection('hg19', exonsEnsemblTrack, gSuite, analysisBins=analysisBins)
    
    intronsEnsemblTrack = ['Genes and gene subsets', 'Introns', 'Ensembl introns']
    intronsOverlap = getRawAndDerivedOverlapResultsForTrackVsCollection('hg19', intronsEnsemblTrack, gSuite, analysisBins=analysisBins)
...
def getCommonRepeatsRawAndDerivedOverlapData(gSuite, analysisBins=None):
    ...
    repeatsCommonOverlap = getRawAndDerivedOverlapResultsForTrackVsCollection('hg19', repeatsCommonTrack, gSuite, analysisBins)
...
def getAllRepeatsProportionOverlapData(gSuite, analysisBins = None, username=''):
    ...
    subtracks = GalaxyInterface.getSubTrackNames(gSuite.genome, ['Sequence', 'Repeating elements'], deep=False, username=username)[0]
    subtrackList = []
...
#     
    ...
    bpsPerDepthLevel = doAnalysis(bpsPerDepthLevelAnalysis, userBin, [Track(track.trackName) for track in gSuite.allTracks()]).getGlobalResult()
...
#     table1Link = generateTableURLFromDataDict(galaxyFn, 'avgTrackVsOtherTable', ['Track'] + colNames, tableData)
    ...
    table1Link = generateTableURLFromDataDict(galaxyFn, 'avgCoverageDepth', ['Coverage depth', 'Average proportion of bps(%)'], table1Data, plotType='pieChart')
    ...
    #     startTime = datetime.now()
        propResultDict = doAnalysis(propDepthAnalysis, userBin, [Track(track.trackName) for track in gSuite.allTracks()]).getGlobalResult()
    #     endTime = datetime.now()
    ...
    table2Link = generateTableURLFromDataDict(galaxyFn, 'avgBpsPerDepth', ['Coverage depth', 'Average coverage for depth for a track (bps)'], tableData, plotType='columnChart')
    ...
    if gSuite.numTracks() < 31:
        vals = vals + tuple([plot1Link])

    vals = vals + tuple([table2Link])
...
def generatePilotPageThreeParagraphs(gSuite, galaxyFn, regSpec='*', binSpec='*'):
    ...
    resultsQ13 = doAnalysis(analysisSpecQ13, userBin, [Track(x.trackName) for x in gSuite.allTracks() ]).getGlobalResult()['Result']
    ...
    tableData = OrderedDict([(gSuite.allTrackTitles()[i], strWithNatLangFormatting(val)) for i, val in enumerate(resultsQ13)])
    table1Link =   generateTableURLFromDataDict(galaxyFn, 'avgCoverageDegree', 
                                                ['Track', 'Average coverage degree of other tracks'], tableData, plotType='columnChart')
    ...
    resultsQ23 = doAnalysis(analysisSpecQ23, userBin, [Track(x.trackName) for x in gSuite.allTracks() ]).getGlobalResult()['Result']
    ...
    tableData = OrderedDict([(gSuite.allTrackTitles()[i], strWithNatLangFormatting(val)) for i, val in enumerate(resultsQ23)])
    table2Link =   generateTableURLFromDataDict(galaxyFn, 'avgUniqueCoverage', 
                                                ['Track', 'Average unique coverage of track'], tableData, plotType='columnChart')
    ...
    resultsQ43 = doAnalysis(analysisSpecQ43, userBin, [Track(x.trackName) for x in gSuite.allTracks() ]).getGlobalResult()['Result']
    ...
    tableData = OrderedDict([(gSuite.allTrackTitles()[i], strWithNatLangFormatting(val)) for i, val in enumerate(resultsQ43)])
    table3Link =   generateTableURLFromDataDict(galaxyFn, 'mostTypical', 
                                                ['Track', 'Average preference for locations shared with other tracks'], tableData, plotType='columnChart')
...
def generatePilotPageFiveParagraphs(gSuite, galaxyFn, regSpec='*', binSpec='*'):
    ...
    avgRK1Kpbs, minRK1Kpbs, maxRK1Kpbs, weakStrong1Kpbs = gSuiteOverview.getGSuiteRipleysKSummaryStatistics(rkResults1Kpbs)
    avgRK1Mpbs, minRK1Mpbs, maxRK1Mpbs, weakStrong1Mpbs = gSuiteOverview.getGSuiteRipleysKSummaryStatistics(rkResults1Mpbs)
    weakStrong1Mpbs = '' if weakStrong1Mpbs == weakStrong1Kpbs else weakStrong1Mpbs
    table7Link = generateTableURLFromDataDict(galaxyFn, 'ripley1kbp', ['Track', 'Ripleys K (1kbp)'], rkResults1Kpbs, plotType='columnChart')
    table8Link = generateTableURLFromDataDict(galaxyFn, 'ripley1mbp', ['Track', 'Ripleys K (1mbp)'], rkResults1Mpbs, plotType='columnChart')
