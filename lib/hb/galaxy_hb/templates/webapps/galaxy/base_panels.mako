...
<%def name="masthead()">
    ...
    ## Workflow tab.
    ${tab( "workflow", _("Workflow"), h.url_for( controller='/workflow', action='index' ), visible=( trans.user ) )}
    ...
    %if app.config.get_bool( 'enable_tracks', False ):
        <%
            ...
            tab( "visualization", _("Visualization"), h.url_for( controller='/visualization', action='list'), menu_options=menu_options, visible=( trans.user ) )
        %>
    %endif
    ...
    <div class="title" style="position: absolute; top: 0; left: 0;">
        ...
        <div style="font-size: 11px; position: absolute; left:409px; top: 11px; width: 200px">$HB_VERSION <a href='http://main.g2.bx.psu.edu/'>(powered by Galaxy)</a></div>
    </div>
