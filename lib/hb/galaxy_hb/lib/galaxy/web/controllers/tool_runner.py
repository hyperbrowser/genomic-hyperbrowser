...
class ToolRunner( BaseUIController ):
    ...
    def index(self, trans, tool_id=None, from_noframe=None, **kwd):
        ...
        # HB morj: if no params then this is a direct request/link to open the tool interface, redirect hyperbrowser-tools to the hyper-controller
    ...
    def rerun( self, trans, id=None, from_noframe=None, **kwd ):
        ...
        if tool.action == '/hyper':
            return trans.response.send_redirect(url_for(controller='hyper', action='index', mako=params_objects.get('mako'), rerun_hda_id=id))
    ...
    def upload_async_message( self, trans, **kwd ):
        ...
        <p>If using the Basic mode in GSuite tools, click <a href="%s">here to return to the Basic mode question list</a></p>""" \
            % url_for( "/static/welcome.html?tabOn=tab1")
