...
class Tool:
    ...
    def check_workflow_compatible( self ):
        ...
        if self.has_multiple_pages:
            return False
        # HyperBrowser tools
        if self.tool_type == "hyperbrowser":
            return True
...
class HyperBrowserGenericTool( HyperBrowserTool ):
    ...
    def execute( self, trans, incoming={}, set_output_hid=True, history=None, **kwargs ):
        ...
        if incoming.has_key('extra_output'):
            try:
                extra_output = simplejson.loads(urllib.unquote(incoming['extra_output']))
...
tool_types = {}
for tool_class in [ Tool, DataDestinationTool, SetMetadataTool, DataSourceTool, AsyncDataSourceTool, HyperBrowserTool, HyperBrowserGenericTool, HyperBrowserMultiGenericTool]:
